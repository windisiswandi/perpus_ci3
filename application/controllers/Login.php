...
class Login extends CI_Controller {
    ...
    public function auth()
    {
        ...
        // auth
        $proses_login = $this->db->query("SELECT * FROM tbl_login WHERE user='$user'");
        if($proses_login->num_rows() > 0)
        {
            $hasil_login = $proses_login->row_array();
            if (md5($pass) === $hasil_login['pass']) {  
                // create session
                ...
                $this->session->set_userdata("errPass", "Password salah");
                redirect("Login");
            }
        }else{
            $this->session->set_userdata("errUser", "Username salah");
            redirect("Login");
        }
    }
